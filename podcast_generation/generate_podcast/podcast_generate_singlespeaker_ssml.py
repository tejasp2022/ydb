...
def generate_podcast():
    ...
    # Check if the content contains SSML tags
    is_ssml = bool(re.search(r'<speak>', text))
    print("Detected SSML: ", is_ssml)
    ...
    if use_long_audio_api:
        ...
        generate_long_audio(text, is_ssml)
    else:
        print("Using standard Text-to-Speech API...")
        generate_standard_audio(text, is_ssml)


def generate_standard_audio(text, is_ssml):
    """Generate audio using the standard Text-to-Speech API for shorter content."""
    credentials, project = default()
    client = texttospeech.TextToSpeechClient(credentials=credentials)
    
    # Configure the synthesis input based on content type
    if is_ssml:
        input_text = texttospeech.SynthesisInput(ssml=text)
        print("Using SSML for enhanced speech synthesis")
    else:
...
def generate_long_audio(text, is_ssml):
    ...
    # Configure the synthesis input based on content type
    if is_ssml:
        input_text = texttospeech_long.SynthesisInput(ssml=text)
        print("Using SSML for enhanced speech synthesis with Long Audio API")
    else:
